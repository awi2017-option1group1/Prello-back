...
export class List {
    static async getAllFromBoardId(req: express.Request, res: express.Response) {
        try {
            ...
            if (boardId == null) res.status(400).json({ message: "Request parameter invalide"}) 
    ...
    static async getOneById(req: express.Request, res: express.Response) {
        try {
            ...
            if (listId == null) res.status(400).json({ message: "Request parameter invalide"}) 
            //else if (forbidden : if the user appratient à la team alors c'est bon) res.status(403)
            //else if (Method Not Allowed) res.status(405)
            else {
    ...
    static async insertFromBoardId(req: express.Request, res: express.Response) {
        try {
            var boardId = req.params.board_id
            if (boardId == null) res.status(400).json({ message: "Request parameter invalide"}) 
            //else if aucune list a inserer
            //else if (forbidden : if the user appratient à la team alors c'est bon) res.status(403)
            //else if (Method Not Allowed) res.status(405)
            else {
                const list = await ListFacade.insertFromBoardId(req.params.board_id)
                res.status(200).json(list)
            }
        } catch (e) {
            res.status(404).json({ message: e.message})
    ...
    static async update(req: express.Request, res: express.Response) {
        try {
            ...
            //else if (Method Not Allowed) res.status(405)
                ...
                const list = await ListFacade.update(req.params.list_id)
                if (list.isEmpty) res.status(200).json(list)
                else res.status(202).json({ message: "No content"})
            }
        } catch (e) {
            res.status(404).json({ message: e.message})
        }
