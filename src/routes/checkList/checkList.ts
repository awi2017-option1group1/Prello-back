...
export class CheckList {
    ...
    static async getAllFromCardId(req: express.Request, res: express.Response) {
        try {
            if (isInteger(req.params.cardId)) {
                const checkList = await CheckListFacade.getAllFromCardId(req.params.cardId)
    ...
    static async insertFromCardId(req: express.Request, res: express.Response) {
        try {
            if (isInteger(req.params.cardId)) {
                const checkList = await CheckListFacade.insertFromCardId(req.params.cardId, req.body)
    ...
    static async update(req: express.Request, res: express.Response) {
        try {
            if (isInteger(req.params.checklistId)) {
                const checkList = await CheckListFacade.update(req.params.checklistId, req.body)
                res.status(200).json(checkList)
            } else {
                res.status(400).json({ error: 'Invalid request parameter' })
            }
    ...
    static async delete(req: express.Request, res: express.Response) {
        try {
            if (isInteger(req.params.checklistId)) {
                await CheckListFacade.delete(req.params.checklistId)
                res.status(204).end()
            } else {
                res.status(400).json({ error: 'Invalid request parameter' })
            }
